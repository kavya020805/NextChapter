...
test('Search for book', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Search books, authors,' }).click();
  await page.getByRole('textbox', { name: 'Search books, authors,' }).fill('horror');
  await page.getByRole('textbox', { name: 'Search books, authors,' }).press('Enter');
});
...
test('Apply filter', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Password' }).fill('nwuser');
  await page.getByRole('button', { name: 'Sign In' }).click();
  await page.getByRole('button', { name: 'Filter and genres menu' }).click();
